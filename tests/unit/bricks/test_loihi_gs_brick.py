...
def test_loihi_gs_preprocessing_and_mapping():
    ...
    # Forward synapses should exist with delay 0
    assert G.has_edge(neuron_A, neuron_aux)
    assert G[neuron_A][neuron_aux]['delay'] == 0

    assert G.has_edge(neuron_aux, neuron_B)
    assert G[neuron_aux][neuron_B]['delay'] == 0

    # Backward synapses should have delays equal to cost-1
    # For A->aux (cost 1) backward delay is 0
    assert G.has_edge(neuron_aux, neuron_A)
    assert G[neuron_aux][neuron_A]['delay'] == 0

    # For aux->B (cost 2) backward delay is 1
    assert G.has_edge(neuron_B, neuron_aux)
    assert G[neuron_B][neuron_aux]['delay'] == 1

    # Edge A->C had cost 1 and should be unchanged (forward delay 0, backward delay 0)
    assert G.has_edge(neuron_A, neuron_C)
    assert G[neuron_A][neuron_C]['delay'] == 0
    assert G.has_edge(neuron_C, neuron_A)
    assert G[neuron_C][neuron_A]['delay'] == 0
